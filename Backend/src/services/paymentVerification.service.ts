import { verifyEtConfig } from '../configs/verifier';
 ...
 * Adapted from TrustPay core verification service
 */
    ...
    try {
      ...
      const response = await fetch(url, {
        ...
        headers: {
          ...
          'Idempotency-Key': `nydl-${params.reference}-${Date.now()}`,
        },
      ...
      if (!response.ok && response.status !== 202) {
        console.error('Verify.ET API Error:', { status: response.status, body });
      ...
      if (response.status === 202) {
        ...
        if (!statusUrl) {
          console.error('Verify.ET 202 response missing statusUrl', body);
          return this.failedResult(params.provider, params.reference, body);
        }
      ...
      console.error('Verify.ET Request Error:', error);
  ...
  private static async pollForResult(
    ...
    for (let attempt = 0; attempt < maxPollAttempts; attempt++) {
      ...
      if (processingStatus === 'failed') {
        console.warn('Verify.ET verification failed during polling', body);
    ...
    console.error('Verify.ET polling timeout', { statusUrl, maxPollAttempts });
    return this.failedResult(provider, reference, { error: 'Polling timeout' });
  }

  /**
   * Map any Verify.ET response to VerifiedTransaction.
   */
