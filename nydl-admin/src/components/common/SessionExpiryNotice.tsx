...
/**
 * Watches the admin JWT's expiry and, at ≤2 min left, shows a banner with a
 * live countdown and a Refresh button that rotates the access token via the
 * refresh cookie — so an admin mid-task isn't logged out unexpectedly.
 */
export function SessionExpiryNotice() {
  ...
  return (
    <div className="fixed inset-x-0 bottom-4 z-[70] flex justify-center px-4">
      <div className="flex w-full max-w-md items-center gap-3 rounded-xl border border-amber-400/50 bg-amber-950/80 px-4 py-3 shadow-lg backdrop-blur">
        <Clock className="size-5 shrink-0 text-amber-400" />
        <div className="min-w-0 flex-1">
          {expired ? (
            <p className="text-sm font-semibold text-amber-200">Your session has expired.</p>
          ) : (
