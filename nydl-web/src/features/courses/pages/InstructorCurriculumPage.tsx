...
export default function InstructorCurriculumPage() {
  ...
  const { data: curriculumRes, isLoading: curriculumLoading } = useCurriculumDetail(id || '');
  const mutations = useCurriculumMutations(id || '');

  const course = courseRes?.data;
  const curriculums = curriculumRes?.data || [];
  ...
  const handleSaveModule = async () => {
    ...
    try {
      if (activeDialog?.type === 'create_module') {
        // Handle curriculum initialization if none exists
  ...
  const handleDeleteModule = async (moduleId: string) => {
    if (!window.confirm('Are you sure you want to delete this module and all of its lessons?')) return;
    try {
  ...
  const handleSaveLesson = async () => {
    ...
    try {
      if (activeDialog?.type === 'create_lesson') {
        ...
        await mutations.updateLesson({
          id: activeDialog.lesson.id,
          moduleId: activeDialog.moduleId,
          title: dialogForm.title,
          description: dialogForm.description || '',
          lessonType: dialogForm.lessonType,
  ...
  const getLessonIcon = (type: string) => {
    switch (type) {
      case 'VIDEO': return <Video className="h-4 w-4 text-rose-450" />;
      case 'LIVE': return <Users className="h-4 w-4 text-emerald-450" />;
      case 'PRACTICE': return <Target className="h-4 w-4 text-blue-450" />;
      case 'QUIZ': return <HelpCircle className="h-4 w-4 text-amber-450" />;
    ...
    <div className="w-full max-w-5xl mx-auto px-4 md:px-8 py-8 space-y-6">
      ...
      <div>
        <Button 
          ...
          className="text-zinc-400 hover:text-white pl-0 gap-1.5"
        >
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-zinc-800 pb-6">
        <div>
          {course && (
            <Badge className="bg-primary/10 border-primary/20 text-primary mb-2">
              {course.code}
            </Badge>
          )}
          <h1 className="text-3xl font-bold tracking-tight text-white">
            {course ? `${course.title} Curriculum` : 'Manage Curriculum'}
          </h1>
          <p className="text-muted-foreground mt-1 text-sm">
            Customize learning path, outline chapters, lectures, and resource files.
          </p>
        </div>
        ...
        {curriculum && (
          <Button 
            ...
            className="bg-primary hover:bg-primary/90 text-primary-foreground font-semibold flex items-center gap-1.5 self-start md:self-center"
          >
      ...
      {isLoading ? (
        <div className="flex flex-col items-center justify-center py-32 space-y-2">
          ...
          <span className="text-sm text-muted-foreground">Loading curriculum outlines...</span>
        </div>
      ) : !curriculum ? (
        <div className="text-center py-20 border border-dashed border-zinc-800 rounded-lg">
          <BookOpen className="h-10 w-10 text-muted-foreground mx-auto mb-4" />
          <h3 className="text-lg font-medium text-white mb-2">No Curriculum Found</h3>
          <p className="text-muted-foreground pb-6 text-sm max-w-sm mx-auto">
            This course does not have an active curriculum template in the database.
          </p>
          <Button 
            onClick={async () => {
              try {
                // Instantly query endpoint to create a default curriculum outline
            ...
            <Card key={mod.id} className="bg-[#09090b] border-zinc-800/80 overflow-hidden">
              ...
              <div className="bg-[#121214] px-6 py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-zinc-850">
                <div>
                  ...
                  <p className="text-xs text-zinc-400 mt-1 lines-clamp-1">{mod.description}</p>
                </div>
                <div className="flex items-center gap-2 self-end sm:self-center">
                  <Button 
                    ...
                    className="text-zinc-300 hover:text-white border border-zinc-800 hover:bg-zinc-900 bg-zinc-950 font-semibold gap-1"
                  >
                  ...
                  <Button 
                    ...
                    className="h-8 w-8 text-zinc-400 hover:text-white hover:bg-zinc-850Dialog"
                  >
              ...
              <CardContent className="p-6">
                {!mod.lessons || mod.lessons.length === 0 ? (
                  <p className="text-zinc-500 text-xs italic py-4 text-center">No lessons in this module. Click 'Add Lesson' to begin.</p>
                ) : (
                      ...
                      <div key={lesson.id} className="py-4 flex flex-col md:flex-row md:items-start justify-between gap-4 first:pt-0 last:pb-0 group">
                        ...
                        <div className="flex items-center gap-1 opacity-80 md:opacity-0 group-hover:opacity-100 transition-opacity self-end md:self-start pt-1">
                          ...
                          <Button 
                            ...
                            className="h-7 w-7 text-zinc-500 hover:text-rose-455 hover:bg-rose-500/10"
                          >
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="space-y-1.5 max-h-32 overflow-y-auto border border-zinc-800 rounded p-2 bg-[#18181b] divide-y divide-zinc-850">
                      {(!dialogForm.resources || dialogForm.resources.length === 0) ? (
                        <p className="text-zinc-500 text-[10px] italic py-2 text-center">No links attached.</p>
                      ) : (
